...
int solve(int n){
    /**
     * dado um valor x e n moedas, calcular o numero de formas distintas
     * de eu conseguir produzir x.
     * 
     * a mesma ideia do problema dos dados
     * 
     * */

    int x; cin >> n >> x;
    vector<int> coins(n); rep(coins);
    vector<int> dp(x+1,0);
    dp[0] = 1;
    for(int i = 1; i <= x; i++){
        for(int c : coins){
